...
d <- data.table(haven::read_sav(file.path(
  RAWmisc::PROJ$RAW,"Neuroticism_health_consumption_171129.sav"
)))
...
for(i in 1:nrow(stack)){
  ...
  fit <- lrm(as.formula(formulas[["formula1adj"]]),
  ...
  options(datadist='ddist0')
  fit <- lrm(as.formula(formulas[["formula1int"]]),
                 x=TRUE, y=TRUE, data=d)
  p0 <- Predict(fit,
               Neuroticism_CA=sort(c(p25,p25_plus_1,p75,seq(250,350,10))),
               ref.zero=T)
...
openxlsx::write.xlsx(table3, file=file.path(
  RAWmisc::PROJ$SHARED_TODAY,"tables","table3_main_effect.xlsx"
))
...
openxlsx::write.xlsx(table4, file=file.path(
  RAWmisc::PROJ$SHARED_TODAY,"tables","table4_stratified_effect.xlsx"
))
