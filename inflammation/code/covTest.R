if(FALSE){
covTest::covTest(a,as.matrix(data),as.numeric(group)-1)
...
newCovTest <- function (fitobj, x, y, sigma.est = "full", status = NULL, maxp = min(nrow(x), 
                                                                                    ncol(x))) 
{
  ...
  yy = y - my
  if (family == "binomial") {
    glmobj = glmnet::glmnet(x, y, family = "binomial", standardize = fitobj$standardize, 
                    lambda.min.ratio = lambda.min.ratio)
  }
  if (family == "cox") {
    glmobj = glmnet::glmnet(x, Surv(y, status), family = "cox", standardize = fitobj$standardize, 
                    lambda.min.ratio = lambda.min.ratio)
  }
  if (family == "cox") {
